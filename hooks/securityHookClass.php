...
namespace hook\security {
  ...
  class securityHookClass implements hookInterface {
    ...
    public static function verifySecurityLink($module, $action) {
      $securityYml = cache::getInstance()->loadYaml(config::getPathAbsolute() . 'config/security.yml', 'securityYml');
      if (isset($securityYml[$module][$action]) === true and isset($securityYml[$module][$action]['credentials']) === true) {
        $credentialsKid = $securityYml[$module][$action]['credentials'];
