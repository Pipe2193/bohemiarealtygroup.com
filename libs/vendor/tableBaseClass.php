...
//          $row = model::getInstance()->prepare($sqlID);
//          $row->execute();
//          $answer = $row->fetchAll(\PDO::FETCH_OBJ);
//          $answer = (integer) $answer->id;
//        model::getInstance()->beginTransaction();
          $query = model::getInstance()->prepare($sql);
          $query->execute();
//        $query->commit();
        } else {
        ...
        foreach ($data as $field => $value) {
          if ($field !== '__sequence') {
            for ($index = 0; $index < count($field); $index++) {
              $params[":" . $field] = ((is_numeric($value) === true) ? $value : $value );
            }
          }
        }
...
//        model::getInstance()->commit();
     ...
     * @throws \PDOException
     */
    public static function getAll($table, $fields, $deletedLogical = true, $orderBy = null, $order = null, $limit = null, $offset = null, $where = null, $bohemiadb = null) {
      try {
...
//
//            if ($flag === false) {
//              $sql = $sql . $field . ' LIKE "%' . $value . '%"';
//              $flag = true;
//            } else {
//              $sql = $sql . ' OR ' . $field . ' LIKE "%' . $value . '%"';
//            }
//          }
//        }
        ...
        if($bohemiadb !== null){
           return model::getBohemiaInstance()->query($sql)->fetchAll(\PDO::FETCH_OBJ);
        }else{
...
//        }
        ...
        foreach ($ids as $field => $value) {
          if ($flag === 0) {
            $sql = $sql . ' WHERE ' . $field . ' = ' . ':' . $field;
            $sqlID = $sqlID . ' WHERE ' . $field . ' = ' . ':' . $field;
          } else {
            $sql = $sql . ' AND ' . $field . ' = ' . ':' . $field;
            $sqlID = $sqlID . ' AND ' . $field . ' = ' . ':' . $field;
          }
          $flag++;
        }
