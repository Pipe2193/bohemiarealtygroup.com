...
//     */
    static public function getBohemiaInstance() {
      ...
      if (session::getInstance()->hasAttribute('mvcIntranetDbQuery')) {
        session::getInstance()->setAttribute('mvcIntranetDbQuery', session::getInstance()->getAttribute('mvcIntranetDbQuery') + 1);
      } else {
        session::getInstance()->setAttribute('mvcIntranetDbQuery', 1);
      }
...
//        self::$instance->setAttribute(\PDO::ATTR_AUTOCOMMIT, FALSE);
     ...
     static private function connectBohemia() {
      try {
        ...
        self::$bohemiaInstance = new \PDO(myConfigClass::getBohemiaDbDsn(), myConfigClass::getBohemiaDbUser(), myConfigClass::getBohemiaDbPassword());
        // PDO::ATTR_ERRMODE: Reporte de errores
        // PDO::ERRMODE_EXCEPTION: Lanza exceptions.
        self::$bohemiaInstance->setAttribute(\PDO::ATTR_EMULATE_PREPARES, false);
        self::$bohemiaInstance->setAttribute(\PDO::ATTR_PERSISTENT, true);
