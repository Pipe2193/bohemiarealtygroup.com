 ...
 */
class landlordTableClass extends landlordBaseTableClass {
   ...
   * @throws PDOException
   */
  public static function getLandlordListingType($landlord_id) {
    try {
      $sql = 'SELECT ' . landlordTableClass::getNameField(landlordTableClass::LISTING_TYPE_ID) . ' AS landlord_listing_type'
              . ' FROM ' . landlordTableClass::getNameTable()
              . ' WHERE ' . landlordTableClass::getNameField(landlordTableClass::DELETED_AT) . ' IS NULL'
              . ' AND ' . landlordTableClass::getNameField(landlordTableClass::ID) . ' = :id';
      $params = array(
      ...
      $answer->execute($params);
      $answer = $answer->fetchAll(PDO::FETCH_OBJ);
      return (count($answer) > 0 ) ? $answer[0]->landlord_listing_type : false;
    } catch (PDOException $exc) {
      throw $exc;
    }
  }
