 ...
 */
class buildingsBaseTableClass extends tableBaseClass {
  ...
  const ADDON_CODES_ZIPCODE = 'zip2';
  const CROSS_ADDRESS = 'cross_address';
  const LATITUDE = 'latitude';
  const LONGITUDE = 'longitude';
  const LOCKBOX_BUILDING = 'lockbox_code';
  const LAUNDRY = 'laundry';
  const ELEVATOR = 'elevator';
  const DOORMAN = 'doorman';
  const AGE = 'age';
  const STORAGE = 'storage';
  const LIVE_IN_SUPER = 'live_in_super';
  const GYM = 'gym';
  const POOL = 'swimming_pool';
  const NOTES_BUILDING = 'notes';
  const OURDOOR_SPACE = 'outdoor_space';
  const BUILDING_TYPE = 'building_type';
  const PETS_POLICY = 'pets';
  const ID_LANDLORD = 'landlord_id';
  const ID_NEIGHBORHOOD = 'neighborhood_id';
  const LISTING_TYPE = 'listing_type';
  const ACCESS = 'access';
  const MAINTENANCE = 'maintenance';
  const REMOVE = 'remove';
  const NESTIO_ID = 'nestio_id';
