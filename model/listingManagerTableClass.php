...
class listingManagerTableClass extends listingManagerBaseTableClass {
   ...
   * @throws PDOException
   */
  public static function getListingManagerIdByUser($usuario_id) {
    try {
      $sql = 'SELECT ' . listingManagerTableClass::getNameField(listingManagerTableClass::USUARIO_ID) . ', '
              . listingManagerTableClass::getNameField(listingManagerTableClass::ID) . ' AS manager_id '
              . ' FROM ' . listingManagerTableClass::getNameTable()
              . ' WHERE ' . listingManagerTableClass::getNameField(listingManagerTableClass::DELETED_AT) . ' IS NULL'
              . ' AND ' . listingManagerTableClass::getNameField(listingManagerTableClass::USUARIO_ID) . ' = :usuario_id';
      $params = array(
      ...
      $answer->execute($params);
      $answer = $answer->fetchAll(PDO::FETCH_OBJ);
      return (count($answer) > 0 ) ? $answer[0]->manager_id : false;
    } catch (PDOException $exc) {
      throw $exc;
    }
  }
