  ...
  public function execute() {
    try {
      if (request::getInstance()->hasGet(listingTableClass::getNameField(listingTableClass::LISTING_HASH, true))) {
        ...
        if (listingTableClass::VerifyListingHash($listing_hash) != false) {
          ...
          listingFeaturesTableClass::update($listing_features_id, $data_listing_features);
          ...
          session::getInstance()->setSuccess("The Listing located at <b> $building_address </b> Apartment # <b> $unit_number </b> under <b>$landlord_name</b> has been Successfully Updated.");
          routing::getInstance()->redirect('listing', 'manage', array(listingTableClass::getNameField(listingTableClass::LISTING_HASH, true) => $listing_hash));
        } else {
          session::getInstance()->setError("The Application encountered an error and it couldn't complete your request.<br> The <b>Listing Token ($listing_hash)</b> is not valid.  ");
