        ...
        if (request::getInstance()->hasGet("hash")) {
          ...
          if ($userID != false) {
            ...
            if (usuarioTableClass::isActivated($userID) != 1) {
              ...
              session::getInstance()->setSuccess(" Congratulations! Email Address <b> $user_email </b> is confirmed, your Account has been successfully Activated.");
              $this->defineView('activation', 'usuario', session::getInstance()->getFormatOutput());
            } else {
              session::getInstance()->setInformation(" Your Account has been successfully activated.Please Log In");
