      ...
      if (request::getInstance()->isMethod('GET')) {
        ...
        if (request::getInstance()->hasGet(usuarioTableClass::getNameField(usuarioTableClass::ACTIVED, true))) {
          ...
          if ($userID != false) {
            ...
            if (usuarioTableClass::isActivated($userID) != 2) {
              $ids = array(
                  usuarioTableClass::ID => $userID
              );
              $data = array(
                  usuarioTableClass::ACTIVED => 2
              );
              usuarioTableClass::update($ids, $data);
              session::getInstance()->setSuccess("<b> $first_name $last_name </b> Account has been successfully Deactivated.");
              routing::getInstance()->redirect("usuario", "index");
            } else {
              session::getInstance()->setInformation("This Account has been Already Deactivated. No Update Required. ");
              routing::getInstance()->redirect('usuario', 'index');
            }
          } else {
