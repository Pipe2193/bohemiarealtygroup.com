 ...
 */
class maintenanceActionClass extends controllerClass implements controllerActionInterface {
      ...
      if (request::getInstance()->isMethod('GET')) {
        ...
        if (request::getInstance()->hasGet(lockboxTableClass::getNameField(lockboxTableClass::LOCKBOX_HASH, true))) {
          ...
          if ($lockbox_id != false) {
            ...
            if (lockboxTableClass::isMaintenance($lockbox_id) == false) {
              ...
              $data = array(
                  lockboxTableClass::LOCKBOX_STATUS => 2
              );
              lockboxTableClass::update($ids, $data);
              $building_address = buildingTableClass::getBuildingAddress($lockbox_hash);
              
              session::getInstance()->setSuccess("<b> The Lockbox from  $building_address </b> has been successfully Maintenance Updated .");
              routing::getInstance()->redirect("lockbox", "index");
            } else {
              session::getInstance()->setInformation("This Lockbox has been Already Maintenance Updated . No Update Required. ");
              routing::getInstance()->redirect('lockbox', 'index');
            }
          } else {
