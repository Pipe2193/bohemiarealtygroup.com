      ...
      if (request::getInstance()->isMethod('GET')) {
        ...
        if (request::getInstance()->hasGet(lockboxTableClass::getNameField(lockboxTableClass::LOCKBOX_HASH, true))) {
          ...
          if ($lockbox_id != false) {
            ...
            if (lockboxTableClass::isActivated($lockbox_id) == false) {

              $ids = array(
                  lockboxTableClass::ID => $lockbox_id
              );
              $data = array(
                  lockboxTableClass::LOCKBOX_STATUS => 1
              );
              lockboxTableClass::update($ids, $data);
              $building_address = buildingTableClass::getBuildingAddress($lockbox_hash);
              
              session::getInstance()->setSuccess("<b> The Lockbox from  $building_address </b> has been successfully Activated.");
              routing::getInstance()->redirect("lockbox", "index");
            } else {
              session::getInstance()->setInformation("This Lockbox has been Already Activated . No Update Required. ");
              routing::getInstance()->redirect('lockbox', 'index');
            }
          } else {
